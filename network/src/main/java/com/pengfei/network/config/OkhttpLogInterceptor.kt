package com.pengfei.network.config

import android.util.Log
import com.pengfei.network.support.HttpUlicodeUtils
import okhttp3.*
import okio.Buffer
 ...
 * 用于记录okHttp的网络日志的拦截器
 */
class OkhttpLogInterceptor(block: (OkhttpLogInterceptor.() -> Unit)? = null) : Interceptor {
     ...
     * 设置LogLevel
     */
    fun logLevel(level: LogLevel): OkhttpLogInterceptor {
        logLevel = level
        return this
    }
     ...
     * 设置colorLevel
     */
    fun colorLevel(level: ColorLevel): OkhttpLogInterceptor {
        colorLevel = level
        return this
    }
     ...
     * 设置Log的Tag
     */
    fun logTag(tag: String): OkhttpLogInterceptor {
        logTag = tag
        return this
    }
        ...
        when (logLevel) {
            ...
            LogLevel.BODY -> {
                ...
                kotlin.runCatching {
                    ...
                    sb.appendln(HttpUlicodeUtils.unicodeDecode(peekBody.string()))
                }.getOrNull()
            }
        }
        sb.appendln("<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<")
